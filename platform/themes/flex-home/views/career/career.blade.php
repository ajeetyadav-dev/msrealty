<div
    class="bgheadproject hidden-xs"
    style="background: url('{{ theme_option('breadcrumb_background') ? RvMedia::url(theme_option('breadcrumb_background')) : Theme::asset()->url('images/banner-du-an.jpg') }}')"
>
    <div class="description">
        <div class="container-fluid w90">
            <h1 class="text-center">{{ __('Careers') }}</h1>
            {!! Theme::partial('breadcrumb') !!}
        </div>
    </div>
</div>
<div class="container padtop50">
    {!! apply_filters('ads_render', null, 'career_detail_before') !!}

    <div class="row">
        <div class="col-sm-9">
            <h2 class="titlenews">{{ $career->name }}</h2>
            <div class="job-list">
                <div class="job-item">
                    <div class="job-header">
                        <p><strong>{{ __('Location') }}:</strong>&nbsp;{{ $career->location }}</p>
                        <p><strong>{{ __('Salary') }}:</strong>&nbsp;{{ $career->salary }}</p>
                        <p><strong>{{ __('Posted at') }}:</strong>&nbsp;{{ Theme::formatDate($career->created_at) }}
                        </p>
                    </div>
                    <div class="job-content">
                        {!! BaseHelper::clean($career->content) !!}
                    </div>
                </div>
            </div>
        </div>
        <div class="col-sm-3">
            {!! apply_filters('ads_render', null, 'career_sidebar_before') !!}

            {!! dynamic_sidebar('primary_sidebar') !!}

            {!! apply_filters('ads_render', null, 'career_sidebar_after') !!}
        </div>
    </div>

    {!! apply_filters('ads_render', null, 'career_detail_after') !!}
</div>
<br>
<br>
