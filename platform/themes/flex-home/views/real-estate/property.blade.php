...
<main class="detailproject bg-white">
    ...
    {!! apply_filters('ads_render', null, 'property_detail_before') !!}
    ...
    <div class="container-fluid w90 padtop20">
        ...
        <p class="addresshouse">
            ...
            <span class="d-inline-block"><i class="fa fa-calendar-alt"></i>
                {{ Theme::formatDate($property->created_at) }}</span>
        </p>
        <div class="d-none">
        ...
        <div class="row">
            <div class="col-md-8">
                                ...
                                <table class="table table-striped table-bordered">
                                    @if ($lastUpdated = $property->getMetaData('last_updated', true))
                                        <tr>
                                            ...
                                            <td>
                                                <strong>
                                                    {{ rescue(fn () => Theme::formatDate($lastUpdated)) }}
                                                </strong>
                                            </td>
                                        </tr>
                                    @endif
                ...
                @endif
                @if(($property->can_see_private_notes ?? false) && ($property->private_notes ?? null))
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="alert alert-primary" role="alert">
                                <h6 class="alert-heading mb-2">{{ __('Private Notes') }}</h6>
                ...
                @if ($property->formatted_floor_plans->isNotEmpty())
                    @include(Theme::getThemeNamespace('views.real-estate.partials.floor-plans'))
                @endif
            ...
            <div class="col-md-4">
                {!! apply_filters('ads_render', null, 'property_detail_sidebar_before') !!}

                {!! apply_filters('property_right_details_info', null, $property) !!}

                @if (!RealEstateHelper::hideAgentInfoInPropertyDetailPage() && ($account = $property->author) && $account->exists)
                    <div class="boxright p-3">
                            ...
                            <div class="col-lg-8 colm10">
                                <div class="info">
                                    <p>
                                        <strong>
                                            @if ($account->url)
                                                <a href="{{ $account->url }}">{{ $account->name }}</a> {!! $account->badge !!}
                                            @else
                        ...
                        @if ($account->whatsapp)
                            <div class="whatsapp-contact-section mt-3 pt-3" style="border-top: 1px solid #e4e4e4;">
                                <div class="text-center mb-2">
                                    <p class="mb-2" style="color: #666; font-size: 14px;">{{ __('Get instant response via WhatsApp') }}</p>
                                </div>
                ...
                {!! apply_filters('ads_render', null, 'property_detail_sidebar_after') !!}
    ...
    {!! apply_filters('ads_render', null, 'property_detail_after') !!}
</main>
