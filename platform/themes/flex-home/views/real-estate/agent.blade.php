    ...
    >
        <div class="description">
            <div class="container-fluid w90">
                <h1 class="text-center">{{ $account->name }} {!! $account->badge !!}</h1>
    ...
    <div class="container-fluid w90 padtop30">
        <div class="rowm10">
            ...
            <div class="agent-details">
                ...
                <div>
                    <h4>{{ $account->name }} {!! $account->badge !!}</h4>
                    @if ($account->email && !setting('real_estate_hide_agency_email', 0) && !$account->hide_email)
                        <p class="mobile mb-2"><strong class="d-inline-block">{{ __('Email') }}</strong>: <span dir="ltr"><a
                                    href="mailto:{{ $account->email }}"
                                >{{ $account->email }}</a></span></p>
                    @endif
                    @if ($account->phone && !setting('real_estate_hide_agency_phone', 0) && !$account->hide_phone)
                        <p class="mobile mb-2"><strong class="d-inline-block">{{ __('Phone') }}</strong>: <span
                                dir="ltr"
                            ><a href="tel:{{ $account->phone }}">{{ $account->phone }}</a></span></p>
                    @endif
                    @if ($account->company)
                        <p class="mobile mb-2"><strong class="d-inline-block">{{ __('Company') }}</strong>: <span
                            >{{ $account->company }}</span></p>
                    @endif
                    <p><strong class="d-inline-block">{{ __('Joined on') }}</strong>: <span
                            class="d-inline-block">{{ Theme::formatDate($account->created_at) }}</span></p>
                    @if ($account->whatsapp)
