...
<main class="detailproject">
    ...
    <div class="container-fluid w90 padtop30 single-post">
        <section class="general">
            ...
            <div class="projecthome mb-2">
                @php
                    $propertiesForPg = app(\Botble\RealEstate\Repositories\Interfaces\PropertyInterface::class)->getPropertiesByConditions(
                        [
                            're_properties.project_id' => $project->id,
                            're_properties.type' => \Botble\RealEstate\Enums\PropertyTypeEnum::PG,
                        ],
                        8,
                        \Botble\RealEstate\Facades\RealEstateHelper::getPropertyRelationsQuery(),
                    );
                ...
                @if ($propertiesForPg->isNotEmpty())
                    <h3 class="headifhouse">{{ trans('plugins/real-estate::project.properties_for_pg') }}</h3>
                    <div class="row rowm10">
                        @foreach ($propertiesForPg as $propertyForPg)
                            <div class="col-sm-6 col-lg-4 col-xl-3 colm10">
                                @include('plugins/real-estate::themes.partials.properties.item', ['property' => $propertyForPg])
                            </div>
                        @endforeach
                    </div>
                @endif
